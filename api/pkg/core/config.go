...
func populateSecrets(secrets *utils.CompendiumSecrets, config *Config) {
...
func loadEnvConfig(flagset *flag.FlagSet) *EnvConfig {
	...
	flagset.StringVar(&secretSource, "compendium_api_secret_source", "", "The source for secrets used to connect with databases and third parties in the application")
	flagset.StringVar(&secretID, "compendium_api_secret_id", "", "In the case the secret source is AWS Secrets Manager, this is the id for the set of secrets")
	flagset.BoolVar(&test, "compendium_env_test", false, "Whether or not the current environment is for automated tests")
	flagset.StringVar(&secretSourceEndpoint, "compendium_api_secret_source_endpoint", "", "The endpoint to connect to the secrets source, used for AWS Secrets Manager")
	return &EnvConfig{
...
func loadGithubConfig(flagset *flag.FlagSet) *GithubConfig {
	...
	flagset.StringVar(&clientID, "compendium_api_github_client_id", "", "The client oauth application id")
	flagset.StringVar(&clientSecret, "compendium_api_github_client_secret", "", "The client oauth application secret")
	return &GithubConfig{
		ClientID:     &clientID,
...
func loadServerConfig(flagset *flag.FlagSet) *ServerConfig {
	...
	var tlsCert string
	var tlsKey string
	flagset.StringVar(&apiPort, "compendium_api_port", "4305", "The port on which the API server should run")
	flagset.BoolVar(&tls, "compendium_api_tls", false, "Whether or not TLS should be used to run the server")
	flagset.StringVar(&tlsCert, "compendium_api_tls_cert", "__local_resources/server.crt", "The path of the file that contains the TLS certificate")
	flagset.StringVar(&tlsKey, "compendium_api_tls_key", "__local_resources/server.key", "The path of the file that contains the TLS key")
	return &ServerConfig{
		APIPort: &apiPort,
		TLS:     &tls,
		TLSCert: &tlsCert,
